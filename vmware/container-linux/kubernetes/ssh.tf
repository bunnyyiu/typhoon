...
resource "null_resource" "cloud-config" {
  ...
  provisioner "file" {
    ...
    destination = "$HOME/vsphere.conf"
  }

  provisioner "remote-exec" {
    inline = [
      "sudo mkdir /etc/kubernetes/cloudprovider/",
      "sudo mv /home/core/vsphere.conf /etc/kubernetes/cloudprovider/vsphere.conf",
    ]
  }
}
...
resource "null_resource" "bootkube-start" {
  ...
  depends_on = ["null_resource.cloud-config", "null_resource.copy-etcd-secrets", "null_resource.copy-kubeconfig"]
